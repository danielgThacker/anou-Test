...
arrowclick = function(cP, type) {
    var r = cP.substring(0, 1)
    var c = cP.substring(1, 2)
...
window.onload = function main() {
    ...
    //load all pictures into container
    //bg_space pic sm_space pic sm_space pic bg_space

    
    for (let i=1; i < numPhotos+1; i++) {
        containerStr += '<p></p>'
        containerStr += '<img class="image" id="' + i +
    ...
    var l_arrow = document.getElementsByClassName("leftArrow")[0]
    var r_arrow = document.getElementsByClassName("rtArrow")[0]
    ...
    l_arrow.addEventListener("click", function() {
        ...
        var newId = arrowclick(currentPic, "left");
        var nextpic = document.getElementById(newId)
        nextpic.click()
    })

    r_arrow.addEventListener("click", function() {
        //console.log("right click")
        var newId = arrowclick(currentPic, "right");
        var nextpic = document.getElementById(newId)
        nextpic.click()
    })
    ...
    Array.from(allPhotos).forEach(photo => {
        photo.addEventListener("click", function() {
            ...
            //check if first picture
            if (currentPic == "00") {
                var larrow = document.getElementsByClassName("leftArrow")[0]
                larrow.style.display = "none";
            } else {
                var larrow = document.getElementsByClassName("leftArrow")[0]
                larrow.style.display = "block";
            }  
            //check if last picture
            if ('"'+currentPic+'"' == maxpic) {
                //console.log("last picture")
                var rarrow = document.getElementsByClassName("rtArrow")[0]
